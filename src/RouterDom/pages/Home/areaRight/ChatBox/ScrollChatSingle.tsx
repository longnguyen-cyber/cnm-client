    ...
    return (
      <>
        ...
        <div style={{
          ...
          left: '50%',
          width: '300px',
          height: `${userClick.senderId === users.id ||userClick.user.id===users.id? '200px' : '100px'}`,
          backgroundColor: '#fff',
          borderRadius: '5px',
          padding: '10px',
          ...
          <div className='flex items-end gap-1'>
            ...
            <p className='text-2xl'>
              <img src={`${imgLaughing}`} style={{ transition: 'width 0.3s, height 0.3s' }} className="hoverEffect" onClick={() => setEmoJi('laugh')} />
            </p>
            <p className='text-2xl'>
              <img src={`${imgSad}`} style={{ transition: 'width 0.3s, height 0.3s' }} className="hoverEffect" onClick={() => setEmoJi('sad')} />
            </p>
            <p className='text-2xl'>
              <img src={`${imgThumbs}`} style={{ transition: 'width 0.3s, height 0.3s' }} className="hoverEffect" onClick={() => setEmoJi('love')} />
            </p>
          ...
          {userClick.user.id === users.id && <button className='flex items-center gap-2 text-red-500 ' onClick={() => { recallMessage(item) }}><BiMessageRoundedX />Thu hồi tin nhắn</button>}
          {userClick.user.id === users.id && <button className='flex items-center gap-2 text-red-500 ' onClick={() => { deletMessage(item) }}> <MdDeleteForever /> xóa tin nhắn</button>}
          {userClick.user.id !== users.id && <button className='flex items-center gap-2 text-red-500 ' onClick={() => { deletMessage(item) }}> <MdDeleteForever /> xóa tin nhắn phía bạn </button>}
        </div>
      </>
    );
  };
      ...
      {Channelid && Channelid.threads ? (
        <div className='content_Scroll' style={{ width: '100%', maxHeight: `${imageUpload && imageUpload.length > 0 ? '70vh' : '85vh'}`, overflowY: 'scroll' }} ref={chatContainerRef}>
          {Channelid.threads
            ...
            <ImageModal isOpen={isModalOpen} onRequestClose={() => setIsModalOpen(false)} src={currentImage} />
                ...
                >
                  ...
                  {m.files ? m.files.map((value: any) => {
                    if (value.filename === 'blob') {
                      return (
                        <audio key={value.filename} controls src={value.path} style={{ width: '300px', height: '50px' }} />
                      );
                    } else {
                      ...
                      if (extension === 'pdf') {
                        ...
                        return (
                          <div key={value.filename}>
                            <iframe src={`https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(value.path)}`} style={{ width: '100%', height: '300px' }} title="Word Document Viewer" />
                            <a href={value.path} download={value.filename}>Download Word</a>
                          </div>
                        );
                      } else {
                        return (
                          <img  onClick={() => openModal(value.path)} key={value.filename} src={value.path} style={{ width: '300px', height: '200px' }} />
                        );
                      }
                    }
                    ...
                    if (value.filename === 'blob') {
                      ...
                      if (extension === 'pdf') {
                        return (
                          <div key={value.filename}>
                             <FaFilePdf style={{ marginRight: '5px' }} /> {/* PDF icon */}
                             <embed src={value.path} type="application/pdf" style={{ width: '100%', height: '20px' }} />
                            <a href={value.path} download={value.filename}><MdFileDownload/>Download PDF</a>
                          </div>
                        );
                      } else if (extension === 'docx' || extension === 'doc') {
                        return (
                          <div key={value.filename}>
                            <iframe src={`https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(value.path)}`} style={{ width: '100%', height: '300px' }} title="Word Document Viewer" />
                            <a href={value.path} download={value.filename}><MdFileDownload/>Download Word</a>
                          </div>
                        );
                      } else {
                        return (
                          <img  key={value.filename} src={value.path} style={{ width: '300px', height: '200px' }} />
                        );
                      }
                    }
                  }) : ""}
                    ...
                    {m.emojis && m.emojis.length > 0 && m.emojis.map((value: any) => {
                      ...
                      return (
                          ...
                          <div className='flex  items-center gap-2'>
                              {value.emoji === 'laugh' && <img src={value.emoji === 'laugh' ? imgLaughing : ''} style={{ width: '20px', height: '20px' }} />}
                              {value.emoji === 'like' && <img src={value.emoji === 'like' ? imgLike : ''} style={{ width: '20px', height: '20px' }} />}
                              {value.emoji === 'angry' && <img src={value.emoji === 'angry' ? imgAngry : ''} style={{ width: '20px', height: '20px' }} />}
                              {value.emoji === 'sad' && <img src={value.emoji === 'sad' ? imgSad : ''} style={{ width: '20px', height: '20px' }} />}
                              {value.emoji === 'love' && <img src={value.emoji === 'love' ? imgThumbs : ''} style={{ width: '20px', height: '20px' }} />}
                          </div>
